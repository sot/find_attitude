...
NSIDE = 16
MIN_ACA_DIST = 25.0 / 3600  # 25 arcsec min separation
MAX_ACA_DIST = 2.1  # degrees corner to corner
MAX_MAG = 10.5  # Max mag to include in the distances file
...
def make_distances_h5(
    ...
    agasc = get_microagasc(microagasc_file)
    dists = get_all_dists(agasc)
    make_h5_file(dists, outfile)


def make_microagasc(outfile=MICROAGASC_FILE):
...
def get_microagasc(filename=MICROAGASC_FILE, date="2025:001"):
...
def get_dists_for_region(stars, ipix):
    ...
    # Get the index of eight potential neighbors. Note that some pixels may have only
    # seven valid neighbors, with the missing neighbor having an index of -1. This is
    # handled correctly by the get_stars_for_region function, which will return an
    # empty table for the missing neighbor.
...
def make_h5_file(t, filename=DISTANCES_FILE):
    ...
    with tables.open_file(filename, mode="a", filters=filters) as h5:
        print("Creating index (takes a few minutes)")
