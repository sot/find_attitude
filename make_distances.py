...
def make_distances_h5(
    max_mag=10.5, outfile="distances.h5", microagasc_file="microagasc.fits"
):
...
def make_microagasc(max_mag=10.5, outfile="microagasc.fits"):
    ...
    t = Table(
        [agasc_id, ra, dec, ra_pm, dec_pm, mag_aca],
...
def _get_dists(s0, s1):
    ...
    dists = sphere_dist(
        s0["ra"][idx0], s0["dec"][idx0], s1["ra"][idx1], s1["dec"][idx1]
    )
...
def plot_dists(agasc, ipix):
    ...
    for dist, id0, id1 in izip(dists, id0s, id1s):
        ...
        dalt = sphere_dist(ra0, dec0, ra1, dec1)
